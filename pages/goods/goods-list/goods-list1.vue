	...
	<view>
		...
		<view class="goods-list">
			<view class="product-list">
				<view class="product" v-for="(goods) in rcommendData" :key="goods.goods_id" @tap="toGoods(goods)">
					<image   :src="goods.img" style="height: 124px;"></image>
					<view class="name">{{goods.goodName}}</view>
					<view class="info">
						<view class="price">￥{{goods.sellPrice}}</view>
	...
	export default {
		...
		methods:{
			...
			recommend:function() {
				console.log(1111111)
				var main = this;
				console.log(url.getFile+"/categoryId")
				uni.request({
					url: url.getFile+"/categoryId",
					method: 'get',
					// dataType: "JSON",
					data:{
						categoryId:main.id,
